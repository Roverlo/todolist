...
try {
    ...
    await body.waitFor();
    const initialState = await store();
    const dateNoteId = initialState.selectedNoteId;
    const archiveDate = initialState.notes.find(note => note.id === dateNoteId).date;
    await body.fill('日期验收：');
    await body.press('Control+End');
    await page.getByRole('toolbar').getByRole('button', { name: '插入日期', exact: true }).click();
    const dateDialog = page.getByRole('dialog', { name: /^(插入|修改)日期$/ });
    await dateDialog.getByLabel('选择日期', { exact: true }).fill('2028-02-29');
    await dateDialog.getByRole('button', { name: '插入日期', exact: true }).click();
    const dateChip = body.locator('time[data-type="noteDate"]');
    assert.equal(await dateChip.getAttribute('datetime'), '2028-02-29');
    await dateChip.click();
    await dateDialog.getByLabel('选择日期', { exact: true }).fill('2032-02-29');
    await dateDialog.getByRole('button', { name: '修改日期', exact: true }).click();
    assert.equal(await dateChip.count(), 1, 'Editing replaces one chip');
    assert.equal(await dateChip.getAttribute('datetime'), '2032-02-29');
    assert.equal(await dateChip.getAttribute('data-weekday'), '周日');
    await page.getByRole('button', { name: '保存', exact: true }).click();
    const savedDateNote = async () => {
        const state = native ? JSON.parse(await readFile(dataPath, 'utf8')).state : await store();
        return state.notes.find(note => note.id === dateNoteId);
    };
    await until(async () => (await savedDateNote())?.content.includes('datetime="2032-02-29"'), 'Date chip persisted');
    assert.match((await savedDateNote()).content, /<time data-type="noteDate" datetime="2032-02-29">2032-02-29<\/time>/);
    assert.equal((await savedDateNote()).date, archiveDate, 'Inline dates do not change the archive date');
    await page.reload();
    await dateChip.waitFor();
    assert.equal((await body.innerText()).trimEnd(), '日期验收：2032-02-29');
    await dateChip.click();
    assert.equal(await dateDialog.getByLabel('选择日期', { exact: true }).inputValue(), '2032-02-29');
    ...
    assert.equal(await dateChip.count(), 0, 'Delete removes the selected date atom');
    await page.keyboard.press('Control+z');
    assert.equal(await dateChip.getAttribute('datetime'), '2032-02-29');
    await page.screenshot({ path: join(output, 'date-chip-reopened.png') });
    console.log('Passed: date chip insert/edit, leap date, weekday, saved HTML, reload, atomic deletion and undo');
    await page.getByRole('button', { name: '一键生成待办事项', exact: true }).click();
