...
try {
    ...
    if ($ClearFormatting) {
        Push-Location (Split-Path -Parent $PSScriptRoot)
        try {
            & node (Join-Path $PSScriptRoot 'test-note-formatting.mjs') --cdp $nativeDebugPort --data $dataPath
    ...
    if ($WeeklyReport) {
        Push-Location (Split-Path -Parent $PSScriptRoot)
        try {
            & node (Join-Path $PSScriptRoot 'test-weekly-report.mjs') --cdp $nativeDebugPort --data $dataPath
            if ($LASTEXITCODE -ne 0) { throw 'Packaged weekly report check failed' }
        } finally { Pop-Location }
    }
    if ($WindowLifecycle) {
        Push-Location (Split-Path -Parent $PSScriptRoot)
