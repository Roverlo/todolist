...
try {
    ...
    assert.equal(await panel.isVisible(), true, 'Weekly report is shown by default');
    assert.equal(await page.getByRole('button', { name: '一键生成周报', exact: true }).getAttribute('aria-pressed'), 'true');
    await page.waitForTimeout(300);
    assert.equal(requests.length, 0, 'Showing the default panel does not send notes to AI');
    ...
    const editedReport = await content.inputValue();
    const collapse = page.getByRole('button', { name: '收起 AI 面板', exact: true });
    await collapse.focus(); await collapse.press('Enter');
    assert.equal(await panel.isVisible(), false);
    const expandedEditor = await page.getByRole('region', { name: '随记编辑区' }).boundingBox();
    assert.ok(expandedEditor.width >= editorBounds.width + 319, 'Collapsing releases the entire side panel width');
    await page.screenshot({ path: `${output}/collapsed.png` });
    const expand = page.getByRole('button', { name: '展开 AI 面板', exact: true });
    assert.equal(await expand.getAttribute('aria-expanded'), 'false');
    await expand.press('Enter');
    assert.equal(await content.inputValue(), editedReport, 'Edited report survives collapse and expand');
    assert.equal(requests.length, before + 1, 'Collapse and expand must not regenerate');
    await page.getByRole('button', { name: '收起 AI 面板', exact: true }).click();
    await open();
    assert.equal(await content.inputValue(), editedReport, 'The active AI action also reopens the retained result');
    assert.equal(requests.length, before + 1);
