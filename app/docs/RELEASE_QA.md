...
| E09 | 工具栏与 AI 分区布局 | 普通编辑工具保持两行；图片/表格上下文可增加一行。工作区宽于 1200px 时右侧 AI 分区与面板对齐，更窄时 AI 操作独占紧凑行；两个入口始终同级并列，切换不挤动顶栏。排序文字完整可见，工具不重叠或截断 |
...
| E13 | 日期标签插入/修改、撤销/重做、保存重开 | 自选日期保存为带完整 ISO 日期文字的 `time` 标签；星期正确，整块删除可撤销，重开仍可修改，随记归档日期不变；周报请求保留日期文字及独立的实际完成时间 |
...
待办联动、完成时间、排序及列表合并可单独运行 `node scripts/test-note-completion.mjs` 检查生产网页构建，或运行 `./scripts/test-portable.ps1 -Executable '<被测 EXE 绝对路径>' -NoteCompletion` 检查隔离原生数据的写入与重载。这两个入口还覆盖日历定位、跨日期新建、搜索/标签组合与清除、回收站计数及重载；`npm run test:notes` 包含编辑器回归。排序使用两个固定显示的文字按钮“未完成在前”“已完成在前”，不随光标进入或离开待办而隐藏，支持键盘操作。普通正文中点击整理本范围待办；无待办时明确提示，引用与单元格不影响范围外的待办；已排序、全部同一状态、单项和空待办时两个按钮仍可点击，给出无需调整的提示且不改变正文或增加撤销步骤。1100px 窗口仍完整可见。删除间隔后相邻同级待办应合并，间距与单个列表一致；撤销恢复间隔，子项、格式和完成时间保持，仍有段落或位于不同父项/单元格的列表保持独立。

工具栏按操作分组：第一行依次为撤销/重做、字体、文字样式、引用/代码等内容样式、格式刷/清除/查找；第二行依次为待办与两个排序按钮、普通列表与减少/增加缩进、对齐与行距、链接/图片/附件/表格/日期。图片和表格的上下文工具位于第一行右侧，AI 操作独立分区、两个等宽入口横向并列；“引用”使用文字按钮。验收光标在普通正文、待办、标题和表格之间切换时排序按钮始终可见；检查 1100、1186、1280、1538、1920px 下控件完整可达、AI 模式切换不导致布局跳动。
