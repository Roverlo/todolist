...
随记标题右侧显示紧凑的“待办事项”统计：总数、已完成、未完成及完成进度，仅统计本篇随记，包含所有层级的子待办。没有待办时显示 0；空白待办仍按一个复选框计数。序号位于勾选框前，间距 4px，主项为 `1、2、3`，子项为 `1.1、1.2`，多层继续展开。排序、增删、缩进和撤销/重做后按当前显示顺序自动重新编号；同一范围内被说明文字隔开的列表连续编号，引用与表格单元格独立编号。编号只用于显示，不写入正文或增加撤销步骤，统计也不会修改待办事项页面的数据。

工具栏“待办”旁的“未完成在前”和“已完成在前”固定显示、始终可点击。光标在普通正文时整理该随记最外层待办；光标在待办时整理当前层级，表格单元格和引用内只整理本范围。没有待办时明确提示；已经排好序、全部同一状态或只有一项时，提示当前顺序无需调整，不修改正文或增加撤销步骤。排序包含被空行或说明文字隔开的同级列表，相同状态保持原有顺序，子待办连同父项一起移动，说明文字保留原位，不跨父项或表格单元格混排。顺序会随正文自动保存，可用 Ctrl+Z 撤销。勾选完成时保持当前位置，需要整理时再点击排序。
