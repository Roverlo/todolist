...
- EXE 交付目录 `portable/01_Offline_Portable/` 只保留最新 4 个 `ProjectTodo_YYYYMMDD_HHmm.exe`。新版验收通过后按文件名中的构建时间清理更早 EXE；先核对绝对路径和保留名单，只删除该目录下符合命名规则的旧 EXE，不递归清理其他目录、用户数据或验收报告。
- 默认直接在 main 上修改、验证、提交和推送；只有用户明确指定其他分支或隔离工作树时才切换。发布与交付包分发仍需单独授权。
