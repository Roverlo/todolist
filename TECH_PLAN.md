...
## 8. 当前已知问题与建议
- 仅本地持久化，缺少用户体系/远端同步。
- 附件/预览、历史与撤销、导入功能未实现或已移除。
- 周期任务仅单日截止，缺少例外/跨周策略。

## 9. 后续排期（建议）
1) 附件落地：本地 fs 存储 + 预览 + 去重（hash）+ 导出 ZIP。
2) 历史/撤销：记录 changes/batchId，提供 Ctrl+Z/Y 与侧栏历史视图。
3) 周期任务增强：多选截止日、例外日期、续期策略。
4) 配置存储抽象：抽象存储层，后续接入远端同步。
5) 主题/密度：预置 light/dark/high-contrast，快捷切换表格密度。
